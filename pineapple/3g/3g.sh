...
*1199:0025*)    echo "Sierra 598u (Ting) detected. Attempting mode switch"
                ...
                ;;
*19d2:0031*)    echo "ZTE MF616 (TIGO-COL) detected. Attempting mode switch"
                uci delete network.wan2         
                uci set network.wan2=interface  
                uci set network.wan2.ifname=ppp0           
                uci set network.wan2.proto=3g           
		...
		iptables -A FORWARD -d 172.16.42.0/24 -m state --state ESTABLISHED,RELATED -i 3g-wan2 -j ACCEPT
                ...
                ;;
*19d2:2003*)    echo "ZTE MF100 (TIGO-COL) detected. Attempting mode switch"
                uci delete network.wan2         
                uci set network.wan2=interface  
                uci set network.wan2.ifname=ppp0           
                uci set network.wan2.proto=3g           
		...
		;;
*19d2:0117*)    echo "ZTE MF668A (COMCEL-COL) detected. Attempting mode switch"
                uci delete network.wan2         
                uci set network.wan2=interface  
                uci set network.wan2.ifname=ppp0           
                uci set network.wan2.proto=3g           
